...
protocol FrameAddressType {
    static func getStackTrace(maxStackSize: Int) -> [String]
}

struct FrameAddress: FrameAddressType {
    static func getStackTrace(maxStackSize: Int = 32) -> [String] {
        var count: Int32 = 0
...
internal func demangleSwiftName(_ mangled: String) throws -> SwiftName {
    return try demangleSwiftName(mangled.unicodeScalars)
}

internal func demangleSwiftName<C: Collection>(_ mangled: C) throws -> SwiftName where C.Iterator.Element == UnicodeScalar {
...
///
/// NAME and ACCESS NOTE: To avoid any dependencies on other files/frameworks, I chose to embed the ScalarScanner class in this file. I made the ScalarScanner private to avoid potential name conflicts. However, this error type might need to be handled externally, so it is internal but I've *renamed* it so that it will not conflict with any internal copies of ScalarScanner in the same namespace.
internal enum DemangleScannerError: Error {
...
internal struct SwiftName: CustomStringConvertible {
    internal let kind: Kind
    internal let children: [SwiftName]
